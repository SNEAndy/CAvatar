...
if __name__ == '__main__':
    ...
    for file in file_list:
        imgs_list = [img for img in os.listdir(os.path.join(opt.input_dir, file)) if img.lower().endswith(('.png', '.jpg', '.jpeg'))]
        for img in imgs_list:
            opt.image_path = os.path.join(opt.input_dir, file, img)
            opt.results_dir = os.path.join(opt.output_dir, file, os.path.splitext(img)[0])
            if not os.path.exists(opt.results_dir):
                os.makedirs(opt.results_dir)
            os.environ['CUDA_VISIBLE_DEVICES'] = opt.gpu_ids

...
class AvatarFaceRecon(BaseModel):
    ...
    def save_mesh(self, name):
        ...
        pred_face = self.pred_face.cpu().numpy().squeeze().transpose(1, 2, 0) * random.randint(100, 200)
        pred_face = cv2.cvtColor(pred_face.astype(np.uint8), cv2.COLOR_RGB2BGR)
        h, w, c = pred_face.shape
        # 遍历每个像素
        for i in range(h):
            for j in range(w):
                # 获取当前像素的颜色通道值
        ...
        cv2.imwrite(name.replace(".obj", ".png"), pred_face)

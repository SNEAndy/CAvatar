...
def get_data_path(root='examples'):
    im_path = [os.path.join(root, i) for i in sorted(os.listdir(root)) if i.lower().endswith('png') or i.lower().endswith('jpg')]
...
def read_data(im, lm_5, lm3d_std, to_tensor=True):
...
def align_for_lm(img, five_points):
    ...
    params = loadmat('utils/BBRegressorParam_r.mat')
